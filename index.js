...
class BottomSheet extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      // isOpen = Initial state of the panel; true to render it opened, false otherwise.
  ...
  togglePanel = () => {
    const { contentHeight, isPanelOpened } = this.state;
     /**
   * animationDuration = How long the panel takes to open/close
   * animation = The close/open animation of the panel
   * sliderMinHeight = Min height of the panel
   * onOpen = Function to execute when the panel is opened
   * onClose = Function to execute when the panel is closed
   */
    const {
      animationDuration,
      animation,
  ...
  render() {
    ...
    /**
   * sliderMaxHeight = Max height of the panel
   * wrapperStyle = Custom style for the panel wrapper
   * outerContentStyle = Custom style for the outer content
   * innerContentStyle = Custom style for the inner content
   * lineContainerStyle = Custom style for the line container
   * lineStyle = Custom style for the line
   */
    const { sliderMaxHeight, wrapperStyle, outerContentStyle, innerContentStyle, 
      lineContainerStyle, lineStyle, children, } = this.props;
...
BottomSheet.propTypes = {
  ...
  sliderMinHeight: (props, propName, _) => {
    if (props[propName] > props.sliderMaxHeight) {
      return new Error(
        'slider Min-Height value cannot be greater than slider Max-Height',
      );
    }
  },
